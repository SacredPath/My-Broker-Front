-- Fix investment_tiers table - Add all missing columns
-- This script handles existing tables gracefully

DO $$
BEGIN
    RAISE NOTICE 'Starting to fix investment_tiers table...';
    ...
    BEGIN
        ...
        RAISE NOTICE 'Added investment_period_days column';
    EXCEPTION WHEN duplicate_column THEN 
        RAISE NOTICE 'investment_period_days column already exists';
    END;
    ...
    BEGIN
        ...
        RAISE NOTICE 'Added allocation_mix column';
    EXCEPTION WHEN duplicate_column THEN 
        RAISE NOTICE 'allocation_mix column already exists';
    END;
    ...
    RAISE NOTICE 'investment_tiers table columns updated successfully';
END $$;
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE 'Created active_sort index';
    EXCEPTION WHEN duplicate_table THEN 
        RAISE NOTICE 'active_sort index already exists';
    END;
    
    BEGIN
        CREATE INDEX IF NOT EXISTS idx_investment_tiers_min_amount ON public.investment_tiers(min_amount);
        RAISE NOTICE 'Created min_amount index';
    EXCEPTION WHEN duplicate_table THEN 
        RAISE NOTICE 'min_amount index already exists';
    END;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'handle_updated_at' AND routine_type = 'FUNCTION') THEN
    ...
    ELSE
        RAISE NOTICE 'handle_updated_at function already exists';
    END IF;
END $$;
