...
class StrategiesPage {
  constructor() {
    ...
    if (!this.api) {
      console.warn("StrategiesPage: API client not found on load. Retrying in 500ms...");
  ...
  async getUserPositions() {
    try {
      const { data, error } = await this.api.fetchEdge('positions_list', {
        method: 'GET'
      });
  ...
  async loadStrategies() {
    try {
      ...
      this.strategies = await this.api.fetchStrategiesList();
  ...
  async showConversionPreview(shortfall) {
    ...
    try {
      ...
      const { data, error } = await this.api.fetchEdge('conversion_quote', {
        method: 'POST',
        body: {
          from_currency: 'USDT',
          to_currency: 'USD',
