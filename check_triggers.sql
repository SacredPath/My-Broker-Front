...
-- Analysis of provided user data
SELECT 
    u.id,
    u.email,
    u.phone,
    ...
    u.app_metadata
FROM auth.users u
WHERE u.id = 'd8cd5ee9-5739-4a82-bf06-7a32aff38186';

-- Check if this user has any recent failed registrations
SELECT 
    COUNT(*) as failed_registrations
FROM auth.users u
WHERE u.email = 'brightmccain@gmail.com'
    AND u.created_at >= NOW() - INTERVAL '7 days'
    AND u.phone = '+234816551932';
...
SELECT 
    ...
    ARRAY_AGG(phone) as phone_list
FROM auth.users 
WHERE phone LIKE '%2348%' AND phone != '+234816551932'
...
SELECT 
    ...
    COUNT(*) as count
FROM auth.users 
WHERE email LIKE '%brightmccain%' AND created_at >= NOW() - INTERVAL '30 days';

-- Check for any users with similar name patterns
SELECT 
    raw_user_meta_data,
    COUNT(*) as count
FROM auth.users 
WHERE raw_user_meta_data::text LIKE '"sub": "d8cd5ee9-5739-4a82-bf06-7a32aff38186"%' AND created_at >= NOW() - INTERVAL '30 days';
