-- Final fix for investment_tiers table
-- Handles unique constraint and data insertion properly
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE public.investment_tiers ADD CONSTRAINT investment_tiers_name_unique UNIQUE (name);
...
-- Insert default data - use simple INSERT without ON CONFLICT
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM public.investment_tiers LIMIT 1) THEN
        ...
        (
            ...
            0.1,
            1,
            '["Basic trading signals", "Email support", "Daily ROI payouts"]'::jsonb,
            '{"BTC": 40, "ETH": 30, "USDT": 30}'::jsonb
        ),
        (
            'Tier 2',
            ...
            0.0643,
            2,
            '["Advanced trading signals", "Priority support", "Daily ROI payouts", "Portfolio analytics"]'::jsonb,
            '{"BTC": 35, "ETH": 35, "USDT": 30}'::jsonb
        ),
        (
            'Tier 3',
            ...
            0.0357,
            3,
            '["Premium trading signals", "24/7 support", "Daily ROI payouts", "Advanced analytics", "Risk management tools"]'::jsonb,
            '{"BTC": 30, "ETH": 40, "USDT": 30}'::jsonb
        ),
        (
            'Tier 4',
            ...
            0.0333,
            4,
            '["VIP trading signals", "Dedicated account manager", "Daily ROI payouts", "Custom analytics", "API access", "Lower fees"]'::jsonb,
            '{"BTC": 25, "ETH": 45, "USDT": 30}'::jsonb
        ),
        (
            'Tier 5',
            ...
            0.0333,
            5,
            '["Exclusive signals", "Personal advisor", "Daily ROI payouts", "Custom strategies", "Priority API", "Zero fees", "Exclusive events"]'::jsonb,
            '{"BTC": 20, "ETH": 50, "USDT": 30}'::jsonb
        );
        RAISE NOTICE 'Default investment tiers inserted successfully';
    ELSE
        RAISE NOTICE 'investment_tiers table already has data';
    END IF;
END $$;
